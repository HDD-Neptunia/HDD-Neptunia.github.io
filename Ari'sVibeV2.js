...
document.addEventListener("DOMContentLoaded", () => {
  ...
  sparks.forEach((spark) => {
    ...
    function runSpark() {
      // random start + distance
      let start = Math.random() * totalLength * 0.8; // keep away from edges
      let distance = 30 + Math.random() * 80; // how far it will travel
      let speed = 2 + Math.random() * 4; // px per frame

      let traveled = 0;
      spark.style.opacity = 1; // show spark

      function animate() {
        if (traveled >= distance) {
          spark.style.opacity = 0; // vanish
